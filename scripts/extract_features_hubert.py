#!/usr/bin/env python3
"""
Extract HuBERT self‑supervised features for overlapping speech detection.

This script operates analogously to `extract_features_wav2vec2.py` but
uses the HuBERT base model.  It reads mixtures from a synthetic OSD
dataset, resamples them to 16 kHz, passes them through a pretrained
HuBERT model from `torchaudio`, segments the resulting frame‑level
embeddings into overlapping windows, computes binary labels from overlap
annotations, and saves the data into compressed ``.npz`` files.

Usage
-----

```bash
python scripts/extract_features_hubert.py \
...
def process_split(model: torch.nn.Module, split_dir: Path, sample_rate: int,
    ...
    """Process all mixtures in a given split directory using HuBERT."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract HuBERT features for OSD datasets.")
    ...
    parser.add_argument('--sample_rate', type=int, default=16000,
                        help='Sampling rate expected by the HuBERT model (default: 16000).')
    ...
    print(f"Extracting HuBERT features on {device}…")
    # Load pretrained HuBERT base model
    bundle = torchaudio.pipelines.HUBERT_BASE
    model = bundle.get_model().to(device)
    model.eval()
    # Frame alignment constants (approximate)
    ...
    print("Finished extracting HuBERT features.")
