#!/usr/bin/env python3
"""
Extract WavLM self‑supervised features for overlapping speech detection.

This script is analogous to `extract_features_wav2vec2.py` and
`extract_features_hubert.py` but uses the WavLM base model.  It reads
mixtures from a synthetic OSD dataset, resamples them to 16 kHz, runs
a pretrained WavLM model, segments the resulting embeddings into
overlapping windows, computes binary labels for each frame from overlap
annotations, and saves the features, labels and masks into ``.npz``
files under a mirrored directory structure.

Usage
-----

```bash
python scripts/extract_features_wavlm.py \
    --input_root <path/to/generated_osd_dataset> \
    --output_root <path/to/save/ssl_features>
```

Additional options mirror those of the Wav2Vec 2.0 extraction script.
"""
...
# Helper functions (duplicated from extract_features_wav2vec2 to avoid relative imports)
...
def process_split(model: torch.nn.Module, split_dir: Path, sample_rate: int,
    ...
    """Process all mixtures in a given split directory using WavLM."""
    ...
    for wav_path in tqdm(wav_paths, desc=f"{split_dir.name}", unit="file", leave=False):
        ...
        with torch.no_grad():
            outputs = model(wav_tensor)
            feats_tensor = outputs[0] if isinstance(outputs, (list, tuple)) else outputs
            feats = feats_tensor[0].cpu().numpy()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract WavLM features for OSD datasets.")
    ...
    parser.add_argument('--sample_rate', type=int, default=16000,
                        help='Sampling rate expected by the WavLM model (default: 16000).')
    ...
    print(f"Extracting WavLM features on {device}…")
    # Load pretrained WavLM base model
    bundle = torchaudio.pipelines.WAVLM_BASE
    model = bundle.get_model().to(device)
    model.eval()
    # Frame alignment constants
    frame_stride_samples = int(0.02 * args.sample_rate)
    frame_window_samples = int(0.025 * args.sample_rate)
    ...
    print("Finished extracting WavLM features.")
